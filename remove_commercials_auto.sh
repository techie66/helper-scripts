...
##
##        'commercialremover.sh %DIR% %FILE% %CHANID% %STARTTIME% %JOBID% %VERBOSEMODE% %CARDID%'
##
...
echo "Transcoding..."
mkdir /tmp/nuvexport
/home/jacob/bin/hdpvrcutter.pl --recordings=$VIDEODIR --basename=$FILENAME --outfile=$FILENAME --tempdir=/tmp/nuvexport --dest=$VIDEODIR --passwd=9up8LRIR
ERROR=$?
if [ $ERROR -ne 0 ]; then
...
mv $VIDEODIR/$FILENAME.mkv $VIDEODIR/$FILENAME
#HandBrakeCLI --native-language eng --subtitle scan,1 --subtitle-forced 1 --quality 21 --format mkv --input "$VIDEODIR/$FILENAME.mkv" --encoder x264 --aencoder copy --audio-copy-mask dtshd,dts,ac3 --audio-fallback lame --output "$VIDEODIR/$FILENAME" --detelecine
...
python /usr/bin/mythvidexport.py --delete --safe $VERBOSEMODE --logfile=/tmp/export.log $JOBID
exit 0
...
##       to the 'mysql mythconverg' command below.)
        # Fix the database entry for the file 
        cat << EOF | mysql mythconverg --password=9up8LRIR 
UPDATE 
        recorded
SET
        cutlist = 0,
